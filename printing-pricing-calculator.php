...
function ppc_log($message) {
    ...
    $log_file = plugin_dir_path(__FILE__) . 'debug-printing.log';
...
add_filter('wpseo_sitemap_index', function($sitemap_index) {
    ...
    ppc_log('wpseo_sitemap_index filter called');

    global $wpdb;

    $last_mod = $wpdb->get_var("SELECT MAX(updated_at) FROM " . PRODUCT_TABLE . " WHERE status = 'active'");

    

    $sitemap_index .= '<sitemap>

        <loc>' . home_url('/ppc-product-sitemap.xml') . '</loc>

        <lastmod>' . ($last_mod ? mysql2date('Y-m-d\TH:i:s+00:00', $last_mod, false) : date('Y-m-d\TH:i:s+00:00')) . '</lastmod>
...
add_action('template_redirect', function() {
    ...
    // Get the path part of the current request (no query string)
    $path = rawurldecode(parse_url($_SERVER['REQUEST_URI'] ?? '', PHP_URL_PATH));

    // Normalize trailing slash and compare
    if ($path === '/ppc-product-sitemap.xml' || rtrim($path, '/') === '/ppc-product-sitemap.xml') {
        // Send proper headers and render your sitemap
        status_header(200);
        nocache_headers();
        ...
        $products = $wpdb->get_results("SELECT slug, updated_at FROM " . PRODUCT_TABLE . " WHERE status = 'active'", ARRAY_A);


        echo '<?xml version="1.0" encoding="UTF-8"?>' . PHP_EOL;

        // Use Yoast's stylesheet if available, otherwise fallback to main

        echo '<?xml-stylesheet type="text/xsl" href="' . home_url('/main-sitemap.xsl') . '"?>' . PHP_EOL;
        ...
        foreach ($products as $product) {
            ...
            echo '    <loc>' . home_url('/produkt/' . $product['slug'] . '/') . '</loc>' . PHP_EOL;

            if (!empty($product['updated_at'])) {

                echo '    <lastmod>' . mysql2date('Y-m-d\TH:i:s+00:00', $product['updated_at'], false) . '</lastmod>' . PHP_EOL;
