...

{
...

    {
        ...
        add_action('admin_post_ppc_export_products_xml', [__CLASS__, 'export_products_xml']);
...

    {
        ...
        if ($wpdb->get_var("SHOW TABLES LIKE '" . PRODUCT_TABLE . "'") !== PRODUCT_TABLE) {
            ...
            dbDelta("CREATE TABLE IF NOT EXISTS " . PRODUCT_TABLE . " (
                ...
                with_price tinyint(1) NOT NULL DEFAULT 0,
            
                popis_1 text DEFAULT NULL,
            
                popis_2 text DEFAULT NULL,
...

    {
        ...
        $header = [
            ...
            'option_id', 'option_title', 'option_slug', 'option_cost', 'option_image', 'override_price', 'with_price'
    ...
    public static function export_products_xml()
    {
        if (!current_user_can('manage_options')) {
            wp_die('Unauthorized', '403 Forbidden');
        }
        ...
        foreach ($products as $product) {
            ...
            foreach ($product as $key => $value) {
                ...
                if (in_array($key, ['title', 'content', 'popis_1', 'popis_2', 'seo_title', 'seo_description'])) {
                    $node = $item->addChild($key);
                    $node_doc = dom_import_simplexml($node);
                    $no = $node_doc->ownerDocument;
                    $node_doc->appendChild($no->createCDATASection($value));
                } else {
        ...
        $base_dir = $upload_dir['basedir'] . '/ppc/xml/products';
        
        if (!file_exists($base_dir)) {
            wp_mkdir_p($base_dir);
        }

        $file_path = $base_dir . '/products.xml';
        
        // Save the XML to file
        $xml->asXML($file_path);

        // Redirect back with success message
        wp_redirect(admin_url('admin.php?page=ppc-products&xml_exported=1'));
        exit;
    }
