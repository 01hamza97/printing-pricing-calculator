    ...
    public function render_settings_page() {
      global $wpdb;
       if (isset($_POST['ppc_settings_save'])) {
        ...
        update_option('ppc_express_delivery_desc', isset($_POST['express_delivery_desc']) ? wp_kses_post(wp_unslash($_POST['express_delivery_desc'])) : '');
        ...
        update_option('ppc_file_check_desc', isset($_POST['file_check_desc']) ? wp_kses_post(wp_unslash($_POST['file_check_desc'])) : '');
        update_option('ppc_cart_expiry_days', floatval($_POST['ppc_cart_expiry_days']));
        update_option('R2_ACCOUNT_ID', $_POST['R2_ACCOUNT_ID']);
        update_option('R2_ACCESS_KEY_ID', $_POST['R2_ACCESS_KEY_ID']);
        update_option('R2_SECRET_ACCESS_KEY', $_POST['R2_SECRET_ACCESS_KEY']);
        update_option('R2_PUBLIC_BASE', $_POST['R2_PUBLIC_BASE']);
        update_option('R2_BUCKET', $_POST['R2_BUCKET']);
        $discounts = [];
